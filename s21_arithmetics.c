...
static void s21_sub_bit(s21_decimal value_1, s21_decimal value_2,
                        s21_decimal *result) {
  int borrow = 0;
  for (int i = 0; i < 96; i++) {
...
int s21_add(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  if (valueSign_1 == valueSign_2) {
    ...
    while (s21_add_bit(value_1, value_2, result)) {
      ...
      if (current_scale == 0) {
        status =
            valueSign_1 ? 2 : 1; // 1 - слишком большое, 2 - слишком маленькое
        break;
      }
...
int s21_mul(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  while (overflow && scale_result > 0) {
    overflow = 0;
    if (s21_decimal_get_power(value_1) > 0)
      s21_decrease_scale(&value_1, 1);
    else if (s21_decimal_get_power(value_2) > 0)
      s21_decrease_scale(&value_2, 1);
    scale_result--;
    ...
    for (int i = 0; i <= b1; i++) {
      if (s21_get_bit(value_1, i)) {
        ...
        if (s21_get_last_bit(shifted) + i > 95 ||
            s21_add_bit(tmp_result, *s21_left_shift(&shifted, i),
                        &tmp_result)) {
          overflow = 1;
          break;
        }
